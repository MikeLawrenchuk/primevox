...
def save_twin_primes_to_csv(twin_primes: List[Tuple[int, int]], filepath='twin_primes.csv'):
    """
...
def count_twin_primes_in_range(start: int, end: int) -> int:
    ...
    range_twin_primes = [(p1, p2) for p1, p2 in all_twin_primes if p1 >= start and p2 <= end]
...
def interactive_twin_prime_finder():
    ...
    try:
        limit_input = input("Enter positive integer limit to find twin primes: ").strip()
